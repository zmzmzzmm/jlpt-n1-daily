...
if(sets.length!==8)throw new Error("需要 8 套每日练习");
...
const done=new Set(sets.slice(0,7).map(s=>s.id));
if(sets.find(s=>!done.has(s.id))?.id!=="set-08")throw new Error("完成前七套后没有进入新题");
if(sets.filter(s=>s.date==="2026-09-24").length!==1)throw new Error("明日练习日期缺失或重复");
console.log("8 套原创练习检查通过：明日新题唯一，完成前七套后不循环。");
