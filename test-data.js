...
if(sets.length!==5)throw new Error("需要 5 套每日练习");
...
const done=new Set(sets.slice(0,4).map(s=>s.id));
if(sets.find(s=>!done.has(s.id))?.id!=="set-05")throw new Error("完成前四套后没有进入新题");
if(sets.filter(s=>s.date==="2026-09-20").length!==1)throw new Error("明日练习日期缺失或重复");
console.log("5 套原创练习检查通过：明日新题唯一，完成前四套后不循环。");
